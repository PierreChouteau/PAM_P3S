...
def fastmnmf2_split(
    ...
    for epoch in range(n_iter):
        ...
        _ , X_tilde = calculate_X_tilde(X_FTM , Q_FMM )

        likelihood[epoch] = calculate_log_likelihood(
                    X_tilde,
                    Y_FTM,
                    Q_FMM,
                )
        print(
            f"Iteration {epoch + 1}/{n_iter} - Likelihood: {likelihood[epoch]}", end="\r"
        )
    ...
    return separated_spec, W_NFK, E_NFL, U_NLK, H_NKT, Y_FTM, T_NKO, P_NOT, g_NM, Q_FMM, likelihood